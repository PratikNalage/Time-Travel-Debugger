...
def get_global_reprs(frame,variables=()):
    result = {}
    for key,value in frame.f_globals.items():
        try:
            ...
            continue
    locals_and_globals = collections.ChainMap(frame.f_globals,frame.f_locals)
...
class Tracer:
    def __init__(self, target_code_object, write, variables=(), depth=1,
        ...
        self.frame_to_global_reprs = collections.defaultdict(lambda: {})
        self.frame_to_old_global_reprs = collections.defaultdict(lambda: {})
        self.depth = depth
        self.prefix = prefix
        assert self.depth >= 1
    ...
    def trace(self, frame, event, arg):
        ...
        self.frame_to_old_global_reprs[frame] = old_global_reprs = \
                                                self.frame_to_global_reprs[frame]

        self.frame_to_global_reprs[frame] = global_reprs = \
                                            get_global_reprs(frame, variables=self.variables)
        ...
        for key,value in global_reprs.items():
            if key not in old_global_reprs:
                newish_global_reprs[key] = value
            elif old_global_reprs[key] != value:
                modified_global_reprs[key] = value

        newish_string = ('Starting var:.. ' if event == 'call' else
                                                            'New var:....... ')
        for name, value_repr in newish_global_reprs.items():
            self.write('{indent}{newish_string}{name} = {value_repr}'.format(
                **locals()))

        for name, value_repr in modified_global_reprs.items():
            self.write('{indent}Modified global variable:.. {name} = {value_repr}'.format(
                                                                   **locals()))
        ...
        for name, value_repr in modified_local_reprs.items():
            self.write('{indent}Modified local variable:.. {name} = {value_repr}'.format(
                                                                   **locals()))
